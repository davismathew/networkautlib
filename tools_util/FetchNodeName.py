...
class Orion:
    ...
    def __init__(self,URL,username,password,nodeIPaddress):
	    self.baseUrl="https://"+URL+":17778/SolarWinds/InformationService/v3/Json/Query?query=SELECT+n.Caption+,+n.IPAddress+,+n.NodeID+FROM+Orion.Nodes+n+WHERE+n.NodeID+in+(+SELECT+n.NodeID+FROM+Orion.NodeIPAddresses+n+WHERE+n.IPAddress+=+\'"+nodeIPaddress+"\')"
    ...
    def getName(self,URL,username,password,nodeIPaddress):
	self.baseUrl = "https://"+URL+":17778/SolarWinds/InformationService/v3/Json/Query?query=SELECT+n.Caption+,+n.IPAddress+FROM+Orion.Nodes+n+WHERE+n.IPAddress+=+\'"+nodeIPaddress+"\'"	    
	self.credentials = (username, password)	
